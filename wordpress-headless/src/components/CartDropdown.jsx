          ...
          {loading ? (
            ...
            <div className="grid gap-4 md:grid-cols-2">
              {carts.slice(0, 2).map((cart) => {
                ...
                return (
                  <Link key={cart.id} to={`/cart/${cart.id}`} onClick={onClose} className="group block overflow-hidden rounded-xl border border-brand/10 bg-card hover:shadow-lg hover:-translate-y-0.5 transition-all duration-200">
                    ...
                    <div className="p-4">
                      ...
                      <ul className="mt-3 grid gap-1.5 text-xs text-brand/70">
                        {details.features.map((feature) => (
                          <li key={feature} className="flex items-start gap-1.5"><span className="font-bold text-moss" aria-hidden="true">✓</span><span>{feature}</span></li>
                        ))}
                      </ul>
                      <span className="mt-4 inline-flex w-full justify-center rounded-lg bg-solar px-4 py-2 text-center text-xs font-bold uppercase tracking-wide text-brand group-hover:brightness-105 transition">{details.button}</span>
                    </div>
                  </Link>
                );
              })}
