...
export default function CartDetail() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      wpCarts.get(id),
      wpBookings.filter({ cart_id: id, status: 'confirmed' }).catch(() => []),
    ])
      .then(([c, b]) => {
        if (!alive) return;
        setCart(c);
        ...
        <div className="grid lg:grid-cols-2 gap-10 lg:gap-16">
          ...
          <div>
            ...
            <div className="mt-8 inline-flex rounded-full bg-brand/5 p-1">
              {['hourly', 'daily'].map((rt) => (
                <button key={rt} onClick={() => setRateType(rt)} className={`rounded-full px-6 py-2 text-sm font-medium capitalize transition ${rateType === rt ? 'bg-brand text-dune' : 'text-brand/60 hover:text-brand'}`}>{rt}</button>
              ))}
            </div>
            <div className="mt-6 flex items-end gap-3">
              <span className="font-display text-5xl text-brand">${totalPrice.toFixed(0)}</span>
              <span className="text-brand/50 mb-2 text-sm">{units} {rateType === 'hourly' ? (units === 1 ? 'hour' : 'hours') : (units === 1 ? 'day' : 'days')} · ${rate}/{rateType === 'hourly' ? 'hr' : 'day'}</span>
            </div>
            <div className="mt-8 grid sm:grid-cols-2 gap-4">
              <DateField label="Pick-up" value={start} onChange={setStart} />
              <DateField label="Return" value={end} onChange={setEnd} />
            </div>
            {!valid && <p className="mt-3 flex items-center gap-2 text-sm text-destructive"><AlertCircle className="w-4 h-4" /> Return time must be after pick-up.</p>}
            <div className="mt-6 flex items-center gap-3">
              {available > 0 ? (available <= 2 ? (
                <span className="inline-flex items-center gap-2 rounded-full bg-solar/15 text-brand px-4 py-2 text-sm font-medium"><span className="w-2 h-2 rounded-full bg-solar animate-pulse" />Only {available} left for these dates</span>
              ) : (
                <span className="inline-flex items-center gap-2 rounded-full bg-moss/10 text-moss px-4 py-2 text-sm font-medium"><Check className="w-4 h-4" /> {available} available</span>
              )) : (
                <span className="inline-flex items-center gap-2 rounded-full bg-destructive/10 text-destructive px-4 py-2 text-sm font-medium"><AlertCircle className="w-4 h-4" /> Sold out for these dates</span>
              )}
            </div>
            <button onClick={handleBook} disabled={!canBook} className="mt-8 w-full rounded-full bg-solar text-brand font-semibold py-4 hover:brightness-105 transition disabled:opacity-40 disabled:cursor-not-allowed">
...
function DateField({ label, value, onChange }) {
  return (
    <label className="flex flex-col gap-1.5">
      ...
      <input type="datetime-local" value={value} onChange={(e) => onChange(e.target.value)} className="rounded-xl bg-white border border-brand/10 text-brand text-sm px-4 py-3 outline-none focus:ring-2 ring-solar" />
    </label>
  );
}