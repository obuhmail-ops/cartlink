    ...
    <div className="min-h-screen bg-dune">
      ...
      <div className="pt-24 px-6 md:px-10 max-w-xl mx-auto pb-20">
        <div className="flex flex-col items-center text-center mb-10">
          <div className="w-16 h-16 rounded-full bg-moss/10 flex items-center justify-center mb-5"><Check className="w-8 h-8 text-moss" /></div>
        ...
        <div className="rounded-3xl bg-white border border-brand/10 overflow-hidden shadow-sm">
          <div className="bg-brand text-dune px-6 py-5 flex items-center justify-between">
            <div><div className="text-[11px] uppercase tracking-widest text-dune/50">Booking reference</div><div className="font-display text-xl">GC-{ref}</div></div>
            <div className="text-right"><div className="text-[11px] uppercase tracking-widest text-dune/50">Status</div><div className="font-medium text-solar">Confirmed</div></div>
          </div>
          <div className="relative aspect-[16/8] w-full overflow-hidden">
            <Image src={cart.image_url} alt={cart.name} fittingType="fill" className="absolute inset-0 w-full h-full" />
          </div>
          <div className="p-6 space-y-4">
            <div className="flex items-center justify-between"><span className="text-brand/50 text-sm">Vehicle</span><span className="font-display text-brand text-lg">{cart.name}</span></div>
...
function Line({ icon, label, value }) {
  return (<div className="flex items-center justify-between gap-4"><span className="flex items-center gap-2 text-brand/55 text-sm">{icon} {label}</span><span className="text-brand font-medium text-right text-sm">{value}</span></div>);
}