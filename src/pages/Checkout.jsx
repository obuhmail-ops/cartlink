      ...
      <div className="pt-24 px-6 md:px-10 max-w-xl mx-auto pb-20">
        ...
        <form onSubmit={submit} className="mt-8 space-y-5">
          <Field label="Full name">
            <input value={form.customer_name} onChange={set('customer_name')} required className="rounded-xl bg-white border border-brand/10 text-brand text-sm px-4 py-3 outline-none focus:ring-2 ring-solar" placeholder="Jordan Rivera" />
          </Field>
          <Field label="Email">
            <input type="email" value={form.customer_email} onChange={set('customer_email')} required className="rounded-xl bg-white border border-brand/10 text-brand text-sm px-4 py-3 outline-none focus:ring-2 ring-solar" placeholder="you@email.com" />
          </Field>
          <Field label="Phone (optional)">
            <input value={form.customer_phone} onChange={set('customer_phone')} className="rounded-xl bg-white border border-brand/10 text-brand text-sm px-4 py-3 outline-none focus:ring-2 ring-solar" placeholder="+1 (555) 010-2024" />
          </Field>

          <button
            type="submit"
            disabled={!ready || submitting}
            className="w-full rounded-full bg-solar text-brand font-semibold py-4 hover:brightness-105 transition disabled:opacity-40 disabled:cursor-not-allowed flex items-center justify-center gap-2"
          >
...
function Row({ icon, label, value }) {
  return (
    <div className="flex items-center justify-between gap-4">
      <span className="flex items-center gap-2 text-brand/55">{icon} {label}</span>
      <span className="text-brand font-medium text-right">{value}</span>
    </div>
  );
}
