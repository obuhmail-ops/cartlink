...
export default function Admin() {
  ...
  const [editing, setEditing] = useState(null); // cart or EMPTY or null to close

  const load = () => {
    setLoading(true);
    Promise.all([
      base44.entities.Cart.list('-created_date', 100),
  ...
  const saveCart = async (e) => {
    ...
    if (editing.id) {
      await base44.entities.Cart.update(editing.id, data);
    } else {
      await base44.entities.Cart.create(data);
    }
    setEditing(null);
    load();
  };
  ...
  const deleteCart = async (c) => {
    ...
    await base44.entities.Cart.delete(c.id);
    load();
  };
        ...
        <div className="mt-8 inline-flex rounded-full bg-brand/5 p-1">
          {['fleet', 'bookings'].map((t) => (
            <button
              key={t}
              onClick={() => setTab(t)}
              className={`rounded-full px-6 py-2 text-sm font-medium capitalize transition ${tab === t ? 'bg-brand text-dune' : 'text-brand/60 hover:text-brand'}`}
            >
        ...
        {loading ? (
          ...
          <div className="mt-8">
            <button
              ...
              className="inline-flex items-center gap-2 rounded-full bg-solar text-brand font-semibold px-5 py-2.5 text-sm hover:brightness-105 transition mb-6"
            >
                ...
                <div key={c.id} className="rounded-2xl bg-white border border-brand/5 overflow-hidden">
                  ...
                  <div className="p-4">
                    ...
                    <div className="mt-4 flex gap-2">
                      <button onClick={() => setEditing(c)} className="flex-1 inline-flex items-center justify-center gap-1.5 rounded-lg border border-brand/15 text-brand py-2 text-sm hover:bg-brand/5 transition">
                        <Pencil className="w-3.5 h-3.5" /> Edit
                      </button>
                      <button onClick={() => deleteCart(c)} className="inline-flex items-center justify-center rounded-lg border border-destructive/20 text-destructive py-2 px-3 text-sm hover:bg-destructive/5 transition">
                ...
                {bookings.length === 0 ? (
                  ...
                  <tr key={b.id} className="border-t border-brand/5">
                    <td className="px-4 py-3">
                      <div className="font-medium text-brand">{b.customer_name}</div>
                      <div className="text-brand/45 text-xs">{b.customer_email}</div>
                    </td>
                    ...
                    <td className="px-4 py-3">
                      <span className={`text-xs px-2 py-0.5 rounded-full ${b.status === 'confirmed' ? 'bg-moss/10 text-moss' : 'bg-destructive/10 text-destructive'}`}>{b.status}</span>
                    </td>
                    <td className="px-4 py-3 text-right">
                      {b.status === 'confirmed' && (
                        <button onClick={() => cancelBooking(b)} className="text-destructive hover:underline text-xs">Cancel</button>
                      )}
                    </td>
                  </tr>
                ))}
