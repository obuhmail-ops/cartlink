import React, { useEffect } from 'react';
import Navbar from '@/components/Navbar';
import Footer from '@/components/Footer';
import MobileBookingBar from '@/components/MobileBookingBar';
import HowItWorksSection from '@/components/HowItWorksSection';
import FaqSection from '@/components/FaqSection';
import FinalBookingCta from '@/components/FinalBookingCta';
import { Image } from '@/components/ui/image';
import { imageUrl } from '@/lib/images';
import { FAREHARBOR_URL } from '@/lib/booking';
import { trackEvent } from '@/lib/track';

const HERO_IMAGE = imageUrl('b4b242b62_generated_image.png');
const CART_IMAGE = imageUrl('6acf6e567_generated_image.png');

const STEPS = [
  { icon: '🚢', title: 'Reserve Before You Sail', body: 'Book your golf cart online before your cruise arrives in Key West.' },
  { icon: '⚓', title: 'Convenient Pickup', body: 'We arrange pickup near the cruise port so you spend more time exploring.' },
  { icon: '🌴', title: 'Explore Key West', body: 'Make the most of your port day — no waiting, no wasting a minute.' },
  { icon: '🔁', title: 'Return & Sail On', body: 'Return your cart before heading back to your ship, stress-free.' },
];

const SEO_TITLE = 'Key West Cruise Port Golf Cart Rentals | Shore Excursion Cart | Paradise Rentals';
const SEO_DESCRIPTION =
  'Explore Key West on your port day with a premium electric golf cart rental. Convenient cruise port pickup for 4 and 6 passenger carts with Apple CarPlay, long-range batteries, and free delivery. Reserve before you sail.';

export default function KeyWestCruisePortGolfCartRentals() {
  useEffect(() => {
    const previousTitle = document.title;
    const descMeta = document.querySelector('meta[name="description"]');
    const previousDescription = descMeta?.getAttribute('content');
    document.title = SEO_TITLE;
    if (descMeta) descMeta.setAttribute('content', SEO_DESCRIPTION);
    return () => {
      document.title = previousTitle;
      if (descMeta && previousDescription) descMeta.setAttribute('content', previousDescription);
    };
  }, []);

  const handleBook = () => trackEvent('check_availability_click', { location: 'key_west_cruise_port_rentals_page', option: 'cruise-visitors' });

  return (
    <div className="min-h-screen bg-dune">
      <Navbar />

      <main className="pt-28 md:pt-36">
        {/* Hero */}
        <section className="px-6 md:px-10 max-w-6xl mx-auto">
          <header className="text-center max-w-3xl mx-auto">
            <p className="text-xs font-bold uppercase tracking-[0.25em] text-moss">Paradise Rentals KW</p>
            <h1 className="mt-3 font-display text-4xl md:text-6xl text-brand text-balance leading-tight">
              Key West Cruise Port Golf Cart Rentals
            </h1>
            <p className="mt-5 text-base md:text-lg text-brand/70 leading-relaxed">
              Cruising into Key West? Reserve before your cruise and enjoy convenient pickup
              arrangements, then return your cart before heading back to your ship. Explore Key West
              without wasting a minute of your port day.
            </p>
            <a
              href={FAREHARBOR_URL}
              target="_blank"
              rel="noreferrer"
              onClick={handleBook}
              className="mt-8 inline-flex items-center gap-2 rounded-lg bg-solar px-6 py-3 text-sm font-bold uppercase tracking-wide text-brand shadow hover:brightness-110 hover:scale-105 active:scale-95 transition-all duration-200"
            >
              Check Cruise Availability
            </a>
          </header>

          <div className="mt-12 overflow-hidden rounded-2xl border border-brand/10 bg-card shadow-sm">
            <Image
              src={HERO_IMAGE}
              alt="Electric golf cart rental for Key West cruise port visitors"
              fittingType="fill"
              className="aspect-[16/10] w-full object-cover"
            />
          </div>
        </section>

        {/* How it works */}
        <section className="px-6 md:px-10 max-w-6xl mx-auto mt-16">
          <h2 className="font-display text-3xl md:text-4xl text-brand text-balance leading-tight text-center">
            How Cruise Port Rentals Work
          </h2>
          <div className="mt-10 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
            {STEPS.map((step, i) => (
              <div key={step.title} className="rounded-2xl border border-brand/10 bg-card p-6 shadow-sm">
                <span className="text-2xl" aria-hidden="true">{step.icon}</span>
                <p className="mt-3 text-xs font-bold uppercase tracking-wider text-moss">Step {i + 1}</p>
                <h3 className="mt-1 font-display text-xl text-brand">{step.title}</h3>
                <p className="mt-2 text-sm text-brand/70 leading-relaxed">{step.body}</p>
              </div>
            ))}
          </div>
        </section>

        {/* Why cruise rental */}
        <section className="px-6 md:px-10 max-w-6xl mx-auto mt-16">
          <div className="grid gap-8 md:grid-cols-2 items-center">
            <div className="overflow-hidden rounded-2xl border border-brand/10 bg-card shadow-sm">
              <Image src={CART_IMAGE} alt="Spacious 6 passenger electric golf cart for cruise visitors" fittingType="fill" className="aspect-[4/3] w-full object-cover" />
            </div>
            <div>
              <h2 className="font-display text-3xl md:text-4xl text-brand text-balance leading-tight">
                Make the Most of Your Port Day
              </h2>
              <p className="mt-5 text-brand/70 leading-relaxed">
                Port days are short — don't waste yours waiting for shuttles or walking in the heat. A
                premium electric golf cart lets you see more of Key West in less time, from iconic Old
                Town landmarks to the Southernmost Point and the best waterfront spots.
              </p>
              <ul className="mt-6 space-y-3 text-sm text-brand/70">
                <li className="flex items-start gap-2">
                  <span className="font-bold text-moss" aria-hidden="true">✓</span>
                  <span>Reserve online before your cruise arrives</span>
                </li>
                <li className="flex items-start gap-2">
                  <span className="font-bold text-moss" aria-hidden="true">✓</span>
                  <span>Convenient pickup arrangements near the cruise port</span>
                </li>
                <li className="flex items-start gap-2">
                  <span className="font-bold text-moss" aria-hidden="true">✓</span>
                  <span>Choose 4 or 6 passenger electric carts</span>
                </li>
                <li className="flex items-start gap-2">
                  <span className="font-bold text-moss" aria-hidden="true">✓</span>
                  <span>Apple CarPlay, Android Auto & premium soundbar</span>
                </li>
                <li className="flex items-start gap-2">
                  <span className="font-bold text-moss" aria-hidden="true">✓</span>
                  <span>Return your cart before heading back to your ship</span>
                </li>
              </ul>
              <a
                href={FAREHARBOR_URL}
                target="_blank"
                rel="noreferrer"
                onClick={handleBook}
                className="mt-8 inline-flex items-center gap-2 rounded-lg bg-solar px-6 py-3 text-sm font-bold uppercase tracking-wide text-brand shadow hover:brightness-110 hover:scale-105 active:scale-95 transition-all duration-200"
              >
                Book Your Cruise Cart
              </a>
            </div>
          </div>
        </section>

        <HowItWorksSection />
        <FaqSection />
        <FinalBookingCta />
      </main>

      <Footer />
      <MobileBookingBar />
      <div className="h-24 md:hidden" aria-hidden="true" />
    </div>
  );
}