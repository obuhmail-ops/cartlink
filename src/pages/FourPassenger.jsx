...
const FEATURES = [
  { icon: '🚗', title: 'Forward-Facing Seating', body: 'All four seats face forward for a safe, comfortable ride.' },
  { icon: '🔋', title: 'Long-Range Lithium Battery', body: 'Up to 60 miles of range — built for a full Key West day.' },
  { icon: '📱', title: 'Apple CarPlay & Android Auto', body: 'Maps, music and hands-free connectivity on the go.' },
  { icon: '🔊', title: 'Premium Soundbar', body: 'Bluetooth soundbar for island vibes wherever you ride.' },
  { icon: '⚡', title: 'No Charging Stress', body: 'Running low? We come to you and swap your cart.' },
  { icon: '🌴', title: 'Free Delivery & Pickup', body: 'Delivered to your hotel, Airbnb, airport or cruise port.' },
];

const SEO_TITLE = '4-Passenger Golf Cart Rental Key West | Forward-Facing Electric Cart';
const SEO_DESCRIPTION =
  'Rent a premium 4-passenger electric golf cart in Key West. Forward-facing seats, long-range lithium battery, Apple CarPlay, premium soundbar, and free delivery to your hotel, Airbnb, airport or cruise port. Book your 4-seater today.';

export default function FourPassenger() {
  useEffect(() => {
  ...
  const handleBook = () => trackEvent('check_availability_click', { location: 'four_passenger_page', ride_type: '4-passenger' });
      ...
      <main className="pt-28 md:pt-36">
        ...
        <section className="px-6 md:px-10 max-w-6xl mx-auto">
          <header className="text-center max-w-3xl mx-auto">
            ...
            <h1 className="mt-3 font-display text-4xl md:text-6xl text-brand text-balance leading-tight">
              4-Passenger Golf Cart Rental in Key West
            </h1>
            <p className="mt-5 text-base md:text-lg text-brand/70 leading-relaxed">
              Our forward-facing 4-seater electric golf cart is the perfect ride for couples and small
              families exploring Key West. Enjoy a long-range lithium battery, Apple CarPlay, a premium
              soundbar, and free delivery anywhere on the island.
            </p>
            <a
          ...
          <div className="mt-12 overflow-hidden rounded-2xl border border-brand/10 bg-card shadow-sm">
            <Image
              src={CART_IMAGE}
              alt="4-passenger forward-facing electric golf cart in Key West"
              fittingType="fill"
        ...
        <section className="px-6 md:px-10 max-w-6xl mx-auto mt-16">
          <h2 className="font-display text-3xl md:text-4xl text-brand text-balance leading-tight text-center">
            Loaded With Island-Ready Features
          </h2>
          ...
          <div className="rounded-2xl border border-brand/10 bg-card p-8 md:p-12 shadow-sm">
            ...
            <div className="mt-6 grid gap-6 md:grid-cols-2 text-brand/70 leading-relaxed">
              <p>
                ...
                easy, nimble ride around Old Town Key West. With all forward-facing seats, every
                passenger enjoys the same comfortable view — no one rides backward.
              </p>
              <p>
                The compact footprint makes parking and navigating narrow Key West streets effortless,
                while the long-range lithium battery gives you up to 60 miles of island exploration on a
                single charge. Add free delivery and pickup, and your Key West adventure starts the
