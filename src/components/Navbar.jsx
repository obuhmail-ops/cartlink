  ...
  return (
    <>
      <header className="fixed top-0 inset-x-0 z-40 bg-dune/95 backdrop-blur-md border-b border-brand/10">
        <div className="hidden md:grid h-32 max-w-7xl grid-cols-3 items-center px-6 lg:px-10 bg-[hsl(var(--sidebar-primary-foreground))]">
          <nav className="flex items-center justify-start gap-3 lg:gap-5 text-[11px] lg:text-xs font-semibold uppercase tracking-wide whitespace-nowrap text-[hsl(var(--foreground))]">
            ...
            <button onClick={() => scrollTo('arrival-options')} className="hover:text-brand transition [font-family:'Sora',_sans-serif] font-bold text-sm">Ports &amp; Cruises</button>
          </nav>
          <Link to="/" aria-label="Paradise Rentals home" className="flex items-center justify-center">
            <video src={LOGO_VIDEO_URL} aria-label="Paradise Rentals" autoPlay loop muted playsInline className="h-28 w-28 object-cover" />
          </Link>
          <nav className="flex items-center justify-end gap-3 lg:gap-5 text-[11px] lg:text-xs font-semibold uppercase tracking-wide whitespace-nowrap text-[hsl(var(--foreground))]">
