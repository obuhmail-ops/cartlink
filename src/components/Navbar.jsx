      ...
      <header className={`fixed top-[calc(0.75rem+env(safe-area-inset-top))] inset-x-3 md:inset-x-6 z-40 transition-all duration-300 rounded-full ${scrolled ? 'bg-dune/95 backdrop-blur-md shadow-lg border border-brand/10' : 'bg-dune/80 backdrop-blur-sm'}`}>
        ...
        <div className="lg:hidden h-16 px-4 flex items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-2">
            ...
            <a href={FAREHARBOR_URL} target="_blank" rel="noreferrer" onClick={() => trackEvent('check_availability_click', { location: 'navbar_mobile' })} className="rounded-lg bg-solar px-3 py-2 text-xs font-bold text-brand whitespace-nowrap shadow hover:brightness-110 active:scale-95 transition-all duration-200">
              Book Now
            </a>
          </div>
        </div>
