  ...
  return React.Children.map(children, (child) => {
    if (typeof child === 'string') {
      return child.split(/(\s+)/).map((part) => {
        ...
        return <motion.span key={key} variants={wordMotion} className="inline-block font-bold [font-family:'Poppins',_sans-serif] text-5xl text-[hsl(var(--brand))]">{part}</motion.span>;
      });
    }
